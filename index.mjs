...
function renderMarkdown(r) {
  var lines = [];
  lines.push("# Beacon Forensics Report");
  lines.push("");
  lines.push("## Summary");
  lines.push("");
  lines.push("| Field | Value |");
  lines.push("|---|---|");
  lines.push("| Withdrawal Address | `" + r.withdrawal_address + "` |");
  lines.push("| Withdrawal Type | " + r.withdrawal_type + " |");
  lines.push("| Validators Found | " + r.validators_found + " |");
  lines.push("| Suspected Operator | " + r.suspected_operator + " |");
  lines.push("| Pattern | " + r.pattern + " |");
  lines.push("| Confidence | **" + r.confidence + "** |");
  lines.push("");

  if (r.cluster_summary) {
    var cs = r.cluster_summary;
    lines.push("## Cluster Summary");
    lines.push("");
    lines.push("| Metric | Value |");
    lines.push("|---|---|");
    lines.push("| Exact-Match Safes | " + cs.exact_match_safes + " |");
    lines.push("| Withdrawal Addresses | " + cs.exact_match_withdrawal_addresses + " |");
    lines.push("| Total Validators | " + cs.exact_match_validators + " |");
    lines.push("| Estimated Staked ETH | **" + cs.estimated_staked_eth.toLocaleString() + "** |");
    lines.push("");
  }

  if (r.deposit_attribution) {
    var da = r.deposit_attribution;
    lines.push("## Deposit Attribution");
    lines.push("");
    lines.push("| Field | Value |");
    lines.push("|---|---|");
    lines.push("| Method | " + da.method + " |");
    lines.push("| Depositor EOA(s) | " + (da.depositor_eoas.length ? da.depositor_eoas.map(function(a) { return "`" + a + "`"; }).join(", ") : "unresolved") + " |");
    lines.push("| Deposits Matched | " + da.deposit_count_matched + " |");
    lines.push("| Total ETH Deposited | " + da.total_eth_deposited + " |");
    lines.push("");
  }
  ...
  if (r.evidence) {
    lines.push("## Evidence");
    lines.push("");
    lines.push("**Query Safe:** `" + r.evidence.query_safe + "`");
    lines.push("");
    lines.push("**Exact-Match Withdrawal Addresses (" + r.evidence.exact_match_withdrawal_addresses.length + "):**");
    lines.push("");
    for (var i = 0; i < r.evidence.exact_match_withdrawal_addresses.length; i++) {
      lines.push("- `" + r.evidence.exact_match_withdrawal_addresses[i] + "`");
    }
    lines.push("");
    lines.push("**Depositor EOA(s):** " + r.evidence.depositor_eoas.map(function(a) { return "`" + a + "`"; }).join(", "));
    lines.push("");
  }

  if (r.confidence_breakdown) {
    var cb = r.confidence_breakdown;
    lines.push("## Confidence Breakdown");
    lines.push("");
    lines.push("| Signal | Score |");
    lines.push("|---|---|");
    lines.push("| Safe Detected | " + cb.safe_detected + " |");
    lines.push("| Validator Cohort Mapped | " + cb.validator_cohort_mapped + " |");
    lines.push("| Depositor Attributed | " + cb.depositor_attributed + " |");
    lines.push("| Exact Owner-Set Cluster | " + cb.exact_owner_set_cluster + " |");
    lines.push("| Evidence Complete | " + cb.evidence_complete + " |");
    lines.push("| **Total** | **" + cb.total + "** |");
    lines.push("");
  }

  if (r.safe) {
    lines.push("## Safe Configuration");
    lines.push("");
    lines.push("Threshold: " + r.safe.threshold + " of " + r.safe.owner_count);
    lines.push("");
    lines.push("**Owners:**");
    lines.push("");
    for (var j = 0; j < r.safe.owners.length; j++) {
      lines.push((j + 1) + ". `" + r.safe.owners[j] + "`");
    }
    lines.push("");
  }
  ...
  lines.push("");
  lines.push("- Validator signing-key custody is not proven from withdrawal-side evidence alone");
  lines.push("- Partial-overlap Safes (shared by fewer than all owners) are excluded from cluster counts");
  lines.push("- Deposit counts from Blockscout may be incomplete due to pagination; BigQuery provides full counts");
  lines.push("- Operator identity remains unresolved; pattern classification is heuristic-based");
  lines.push("");
  lines.push("---");
  lines.push("*Generated by [beacon-forensics](https://github.com/ivan09069/beacon-forensics)*");
...
async function main() {
  ...
  for (var a = 3; a < args.length; a++) {
    ...
    if (args[a] === "--format" && args[a + 1]) outputFormat = args[a + 1];
  }
